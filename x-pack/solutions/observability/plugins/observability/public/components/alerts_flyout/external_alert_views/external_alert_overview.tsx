 ...
 */
export function ExternalAlertOverview({ alert }: ExternalAlertOverviewProps) {
  const source = (alert[ALERT_SOURCE] as string[] | undefined)?.[0];
  const rawPayload = alert['kibana.alert.raw_payload'] as Record<string, unknown> | undefined;
  const externalUrl = (alert['kibana.alert.external_url'] as string[] | undefined)?.[0];
  const connectorId = (alert['kibana.alert.connector_id'] as string[] | undefined)?.[0];
  const alertTitle = (alert['kibana.alert.rule.name'] as string[] | undefined)?.[0];
  const alertReason = (alert['kibana.alert.reason'] as string[] | undefined)?.[0];
  const severity = (alert['kibana.alert.severity'] as string[] | undefined)?.[0];
  const status = (alert['kibana.alert.status'] as string[] | undefined)?.[0];
  const timestamp = (alert['@timestamp'] as string[] | undefined)?.[0];

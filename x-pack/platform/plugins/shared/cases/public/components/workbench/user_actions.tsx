...
import { getManualAlertIdsWithNoRuleId } from '../user_actions/helpers';
...
import { useUserActionsPagination } from '../user_actions/use_user_actions_pagination';
import { useLastPageUserActions } from '../user_actions/use_user_actions_last_page';
import { ShowMoreButton } from '../user_actions/show_more_button';
...
export const UserActions = React.memo((props: UserActionTreeProps) => {
  ...
  const {
    ...
    handleUpdate,
    loadingCommentIds,
  } = useUserActionsHandler({ caseId: caseData.id });
  ...
  const MarkdownNewComment = useMemo(
    ...
    [caseData.id, handleUpdate, handleManageMarkdownEditId, statusActionButton, commentRefs]
  );
